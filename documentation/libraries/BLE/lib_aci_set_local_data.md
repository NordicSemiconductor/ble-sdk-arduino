...
Events generated
    ACI_EVT_CMD_RSP is generated when the nRF8001 receives the SetLocalData ACI command. 
...
Aruino UNO example:
    In the ble_uart_project_template, the "Hardware Revision string" Bluetooth Characteristic has a "Set Pipe"
    The define value for the pipe is in the services.h<hyperlink> and services_lock.h <hyperlink>.
...
In the example code the device version of the nRF8001 is read using the lib_aci_device_version(), the device version that is read out is stored in a pipe.
